...
void SciTEWin::Creation() {
	...
	TBADDBITMAP addbmp1 = { hInstance, IDR_GO };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp1);

	TBADDBITMAP addbmp2 = { hInstance, IDR_STOPEXECUTE };
	::SendMessage(hwndToolBar, TB_ADDBITMAP, 1, (LPARAM)&addbmp2);
	...
	for (unsigned int i = 0;i < ELEMENTS(bbs);i++) {
		...
		else if (bbs[i].cmd == IDM_GO )
			tbb[i].iBitmap = STD_PRINT + 2;

		else if (bbs[i].cmd == IDM_STOPEXECUTE )
			tbb[i].iBitmap = STD_PRINT + 3;
		else
